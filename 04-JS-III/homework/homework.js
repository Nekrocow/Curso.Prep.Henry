...
function devolverUltimoElemento(array) {
  ...
  // Tu código:
  return array[array.length - 1]; // recordar que .length solo me devuelve un valor por encima del total de datos, porque empieza contando en 0
}
...
function agregarNumeros(numeros) {
  ...
  // Tu código:
  var Suma = 0; // MUY importante: la variable tiene que estar definida ANTES del bucle, si no, el bucle repite la definición.
  for (i = 0; i < numeros.length; i++) {
    Suma += numeros[i];
  }
  return Suma; // return POR FUERA del bucle, corre cuando terminó.
}
...
function mayorACien(array) {
  ...
  let may = [];
  for (let i = 0; i < array.length; i++) {
    if (array[i] > 100) may.push(array[i]);
  }
  return may;
}
